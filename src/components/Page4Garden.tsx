...
import { Flower, Heart, Bird as Butterfly } from 'lucide-react';
...
const Page4Garden = () => {
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-green-100 via-green-200 to-emerald-100 relative overflow-hidden">
      ...
      {butterflies.map((butterfly) => (
        ...
        >
          <Butterfly className="text-pink-400 animate-pulse" size={20} />
        </div>
      ))}
      ...
      >
        <div className="min-h-[150vh] relative p-4 flex flex-col pb-24">
          ...
          <div className="text-center py-16">
            <h1 className="text-3xl md:text-5xl font-light text-pink-600 mb-4">
              Self-Love Garden 🌸
            </h1>
          ...
          <div className="relative w-full flex-grow">
            {/* Grass effect */}
            <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-green-300 to-transparent"></div>
            ...
            {flowers.map((flower, index) => (
              ...
              >
                {/* Stem */}
                <div
                  className={`absolute left-1/2 top-full w-0.5 bg-green-700 transition-all duration-1000 ${
                ...
                {activeFlower === index && bloomedFlowers.has(index) && (
                  <div className="absolute left-1/2 transform -translate-x-1/2 -translate-y-full mb-2 w-48 md:w-64">
                    <div className="bg-pink-50/90 backdrop-blur-sm rounded-2xl p-3 shadow-lg border border-pink-200 animate-fade-in-up">
                      <p className="text-xs md:text-sm text-pink-700 text-center leading-relaxed">
                        {flower.quote}
                      </p>
                      <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-1/2 w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent border-t-pink-50/90"></div>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="text-center py-16 mt-auto">
            <p className="text-pink-600 text-sm md:text-base leading-relaxed max-w-md mx-auto">
              Each flower in your garden represents a part of you that's ready to
              bloom again. You are your own sanctuary. 💖
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="fixed right-4 top-1/2 transform -translate-y-1/2 z-20">
        <div className="w-1 h-16 bg-pink-200/30 rounded-full overflow-hidden">
          <div
            className="w-full bg-gradient-to-b from-pink-500 to-pink-400 transition-all duration-300 ease-out"
            style={{
